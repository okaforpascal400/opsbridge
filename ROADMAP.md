...
A phased plan, sequenced the way a Forward Deployed Engineer delivers: prove the
seam early, deploy early, and weight the effort toward the things that make an agent
*trustworthy in production* — evals, observability, guardrails.
...
## Phase 1 — Foundation & the mess  ·  Hats: 1
...
- ☐ Normalizers: dates (3 formats → ISO), phones (→ E.164-ish), amounts, statuses
- ☐ Fuzzy join: returns → orders on name + phone, with a confidence score
...
- ☐ Session memory/state + a documented context-assembly step
- ☐ MCP server exposing the read tools
- ☐ Tests per tool + a loop-level test with a stubbed model

## Phase 4 — Guardrails & human-in-the-loop  ·  Hats: 2, 3 (safety/security)
...
- ☐ Structured proposal object with rationale + affected records
- ☐ Policy checks (e.g. refund note requires a matched return; hold requires a reason)
...
- ☐ Golden dataset of ops questions + expected answers/behaviours
- ☐ Scoring harness (exact + judged where needed), reproducible via one command
...
## Phase 7 — Package & broadcast  ·  Hats: 3, signals 4 & 5
...
- ☐ README: architecture, evals table, "how to debug it", one-command run + deploy
- ☐ Final Railway deploy verified reproducible
- ☐ LinkedIn post applying Ed's five-hats framework, tag Ed, link repo + demo

---

## Sequencing notes
- Deploy in Phase 1 and keep it deployable every phase after — never leave deploy to the end.
- Phase 2 is written by hand; it's the piece I must defend cold.
- If time compresses, protect Phases 4–6. The agent alone is commodity; the guardrails,
